    ...
    DELAY: DelayNs,
{
    ...
    fn clear_frame(&mut self, spi: &mut SPI, _delay: &mut DELAY) -> Result<(), SPI::Error> {
        ...
        self.interface.data_x_times(spi, 0u8, WIDTH * HEIGHT)?;
...
/// EDP3in7 driver for grayscale
pub struct EPD3in7G<SPI, BUSY, DC, RST, DELAY> {
  /// Connection Interface
  interface: DisplayInterface<SPI, BUSY, DC, RST, DELAY, SINGLE_BYTE_WRITE>,
}

impl<SPI, BUSY, DC, RST, DELAY> InternalWiAdditions<SPI, BUSY, DC, RST, DELAY>
    for EPD3in7G<SPI, BUSY, DC, RST, DELAY>
where
    SPI: SpiDevice,
    BUSY: InputPin,
    DC: OutputPin,
    ...
    DELAY: DelayNs,
{
    fn init(&mut self, spi: &mut SPI, delay: &mut DELAY) -> Result<(), SPI::Error> {
        ...
        self.interface
            .cmd_with_data(spi, Command::BorderWaveformControl, &[0x00])?;
    ...
    DELAY: DelayNs,
{
    ...
    fn update_frame(
        ...
        self.interface.cmd_with_data(spi, Command::Unknown, &[0x00])?;
        ...
        self.interface
            .cmd_with_data(spi, Command::WriteRam, &buffer[..buffer.len() / 2])?;
        ...
        self.interface
            .cmd_with_data(spi, Command::WriteRam2, &buffer[buffer.len() / 2..])?;
    ...
    fn display_frame(&mut self, spi: &mut SPI, delay: &mut DELAY) -> Result<(), SPI::Error> {
        // self.interface.cmd_with_data(spi, Command::WriteLutRegister, &LUT_4GRAY_GC)?;
        self.interface.cmd_with_data(spi, Command::DisplayUpdateSequenceSetting, &[0xC7])?;
        self.interface.cmd(spi, Command::DisplayUpdateSequence)?;
        self.interface.wait_until_idle(delay, IS_BUSY_LOW);
        Ok(())
    }
    ...
    fn clear_frame(&mut self, spi: &mut SPI, _delay: &mut DELAY) -> Result<(), SPI::Error> {
        ...
        self.interface.cmd(spi, Command::WriteRam)?;
        self.interface.data_x_times(spi, 0u8, WIDTH * HEIGHT / 8)?;
    ...
    fn set_lut(
        ...
        let buffer = match refresh_rate {
            Some(RefreshLut::Full) | None => &LUT_4GRAY_GC,
            Some(RefreshLut::Quick) => &LUT_4GRAY_A2,
        };
